...
//     try {
//       const response = await fetch('https://algomate-p45p.onrender.com/api/linked-list/animate', {
//         method: 'POST',
//         headers: {
...
const TraversalVisualizer = () => {
  ...
  const handleRun = async () => {
    ...
    // Mock Animation Data Generation (Replaces backend fetch)
    const generateMockAnimation = (data) => {
        const steps = [];
        steps.push({ codeLine: 2, nodeIndex: 0, description: "Initialize 'current' pointer to head." });
        
        data.forEach((value, index) => {
            steps.push({ codeLine: 3, nodeIndex: index, description: `Checking condition: current (${value}) is not nullptr.` });
            steps.push({ codeLine: 4, nodeIndex: index, description: `Access and process data: ${value}.` });
            steps.push({ codeLine: 5, nodeIndex: index, description: `Move 'current' to the next node.` });
        });

        steps.push({ codeLine: 3, nodeIndex: -1, description: "Condition check: 'current' is now nullptr. Loop terminates." });
        steps.push({ codeLine: 7, nodeIndex: -1, description: "Traversal complete." });

        return { initialState: data, steps: steps };
    };
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (nodeCount > 0) {
        ctx.fillStyle = '#090e0fff';
        ctx.font = `bold ${14 * zoom}px sans-serif`;
        ctx.textAlign = 'center';
        ctx.fillText('head', x, y - scaledNodeHeight / 2 - 25 * zoom);
        ctx.strokeStyle = '#050b0cff';
        ctx.lineWidth = 2 * zoom;
        ctx.beginPath();
        ctx.moveTo(x, y - scaledNodeHeight / 2 - 20 * zoom);
        ctx.lineTo(x, y - scaledNodeHeight / 2);
      ...
      for (let i = 0; i < nodeCount; i++) {
        ...
        ctx.fillStyle = '#ffffff';
        ctx.strokeStyle = isCurrent ? '#EF4444' : '#030406ff';
      ...
      <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
        ...
        <div className="flex flex-col p-6 w-full md:w-2/5 border-r border-gray-200">
          ...
          <div className="bg-white rounded-lg shadow-lg p-4 border border-gray-200">
            ...
            <p className="mt-2 text-sm text-gray-500">
              For a better view, try a small list size.
            </p>
          </div>
        </div>

        {/* Right Panel: Animation & Status */}
        <div className="flex-1 flex flex-col p-6 relative overflow-hidden">
